...
export default function LocationsDashboardPage() {
  ...
  const { data: locationCategoriesResp } = useQuery({
  ...
  const {
    ...
    searchQuery: search,
    searchFields: ["slug", "translations"] as (keyof ApiLocation)[],
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      postAppEvent({ type: "locations/changed" });
      setFormOpen(false);
      setForm(emptyForm());
  ...
  function openEdit(l: ApiLocation) {
    setForm({
      id: l.id,
      slug: l.slug,
      logo: l.logo,
      translations: {
        en: {
          name: l.translations.en.name || "",
        },
        km: {
          name: l.translations.km.name || "",
        },
      },
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    } as Omit<ApiLocation, "id" | "createdAt" | "updatedAt" | "category">;
    ...
    switch (field) {
      case "en.name":
        return form.translations.en.name.trim() === ""
          ? "Location Name (English) is required"
          : "";
      case "km.name":
        return form.translations.km.name.trim() === ""
          ? "Location Name (Khmer) is required"
          : "";
      case "categoryId":
        return form.categoryId.trim() === ""
          ? "Location Category is required"
          : "";
      case "logo":
        return form.logo.trim() === "" ? "Logo is required" : "";
      default:
        return "";
    }
  };
          ...
          {locationsError ? (
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-4">
                There was an error loading the locations. Please try again.
              </p>
              <button
                  ...
                  paginatedLocations.map((l) => (
                    <tr key={l.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-sm text-gray-800">
                        {l.translations.en.name}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-800">
                        {l.translations.km.name}
                      </td>
                      <td className="px-4 py-2">
                        {l.logo && (
                          <img
                            src={l.logo}
                            alt={l.translations.en.name}
                            className="h-10 w-10 object-contain rounded"
                          />
                        )}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-600">
                        {l.category?.translations.en.name || "Unknown"}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-600">
